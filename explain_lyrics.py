...
from utils import (load_prompt_template, 
                   read_file, get_default_llm_config, 
...
def explain_lyrics_content(lrc_content, target_language="Traditional Chinese"):
    ...
    prompt = load_prompt_template(prompt_file_name, target_language=target_language, lyrics_content=lyrics_content)
    ...
    try:
        ...
        # Create Pydantic AI agent for explanation
        agent = Agent(
            openai_model,
            retries=3
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument('-l', '--language', default='Traditional Chinese', help='Target language for explanation')
    ...
    if input_path:
        output_path = args.output or input_path.replace('.lrc', f'_explanation_{args.language.replace(" ", "_")}.txt')
    else:
        output_path = args.output

    success = main(input_path, output_path, args.language, log_level=args.log_level, logfire_enabled=args.logfire)
    if not success:
        exit(1)