...
class ProcessingResults:
    ...
    def to_dict(self) -> dict:
        ...
        return {
            ...
            "story_sources": (
                ", ".join(self.story_sources) if self.story_sources else ""
            ),
...
def process_second_phase(
    ...
    try:
        ...
        try:
            ...
            logger.warning(
                f"Could not read transcript file for song identification: {e}",
                exc_info=True,
            )
        ...
        if not generate_lrc_step(
            lyrics_content, None, transcript_path, paths, resume, results
        ):
            results.finalize()
            return False, results
        ...
        transcript_path = str(paths["transcript_word_txt"])
        if not verify_and_correct_timestamps_step(
            lrc_path, transcript_path, paths, resume, results
        ):
            results.finalize()
            return False, results
        ...
        if not explain_lyrics_step(
            corrected_lrc_path, paths, target_language, resume, results
        ):
            results.finalize()
            return False, results

        # Step 7: Add translation to Traditional Chinese
        if not translate_lrc_step(
            corrected_lrc_path, paths, target_language, resume, log_level, results
        ):
            results.finalize()
            return False, results
...
def extract_metadata_step(input_file: Path, results: ProcessingResults) -> bool:
    ...
    try:
        ...
        logger.info(
            f"Metadata extracted: {metadata.get('title', 'Unknown')} - {metadata.get('artist', 'Unknown')}"
        )

        if not (metadata["title"] and metadata["artist"]):
            logger.warning(
                f"Missing title or artist for {input_file}, cannot search for lyrics"
            )
        return True
...
def separate_vocals_step(
    input_file: Path, paths: dict, resume: bool, results: ProcessingResults
) -> bool:
    ...
    if resume and vocals_path.exists():
        logger.info(
            f"Vocals file already exists for {input_file}, skipping vocal separation..."
        )
...
def transcribe_vocals_step(
    vocals_file: str, paths: dict, resume: bool, results: ProcessingResults
    ...
    try:
        ...
        if segments:
            ...
            logger.info(
                f"Transcription completed: {len(segments)} segments, saved to: {transcript_file}"
            )
            return True
        else:
            results.transcription_success = False
...
def identify_and_search_lyrics_step(
    ...
    try:
        ...
        if has_metadata:
            logger.info(
                f"Using metadata + ASR approach for '{metadata['title']}' by {metadata['artist']}"
            )
            identified_song = identify_song_from_asr(
                transcript_content, paths, force_recompute=not resume, metadata=metadata
            )
        else:
        ...
        if identified_song:
            ...
            if lyrics_content:
                ...
                results.lyrics_source = (
                    lyrics_source_url
                    or f"LLM search (identified from ASR: {native_language})"
                )
                ...
                logger.warning(
                    f"Song identified but no lyrics found for '{song_title}' by {artist_name}"
                )
...
def generate_lrc_step(
    lyrics_content: Optional[str],
    corrected_transcript_content: Optional[str],
    transcript_path: str,
    paths: dict,
    resume: bool,
    results: ProcessingResults,
    ...
    if resume and lrc_path.exists():
        ...
        if lyrics_content:
            ...
            logger.info(
                "No lyrics found, using grammatically corrected transcript for LRC generation..."
            )
            lrc_lyrics = generate_lrc_lyrics(
                corrected_transcript_content, asr_transcript
            )
        else:
            logger.info(
                "No lyrics found and no corrected transcript, converting ASR transcript to LRC format..."
            )
...
def verify_and_correct_timestamps_step(
    ...
    if resume and corrected_lrc_path.exists():
        logger.info(
            f"Corrected LRC file already exists, skipping timestamp verification..."
        )
    ...
    try:
        ...
        if not asr_transcript:
            ...
            results.error_message = (
                "Could not read ASR transcript for timestamp verification"
            )
            logger.exception("ASR transcript file is empty or could not be read")
            return False

        # Verify and correct timestamps
        corrected_lrc_content = verify_and_correct_timestamps(
            lrc_content, asr_transcript
        )
        ...
        if corrected_lrc_content:
            ...
            logger.info(
                f"LRC timestamps verified and corrected, saved to: {corrected_lrc_path}"
            )
            ...
            results.timestamp_verification_success = False
            results.error_message = (
                "Timestamp verification returned no corrected content"
            )
...
def translate_lrc_step(
    ...
    translated_lrc_content = translate_lrc_content(
        read_file(str(lrc_path)), target_language
    )
...
def explain_lyrics_step(
    ...
    try:
        ...
        if explanation_content:
            ...
            logger.info(
                f"Lyrics explanation completed and saved to: {explanation_path}"
            )
            return True
        else:
            results.explanation_success = False
...
def process_single_audio_file(
    ...
    if paths["translated_lrc"].exists() and resume:
        logger.info(
            f"Final LRC file already exists for {input_file}, skipping processing..."
        )
        ...
        if json_file_path.exists():
            try:
                ...
                logger.info(
                    f"Loaded song info from JSON: {results.metadata_title} - {results.metadata_artist}"
                )
